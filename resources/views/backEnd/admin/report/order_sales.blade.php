            ...
            <div class="row excel-export d-flex justify-content-between">
                <div class="col-md-6 col-12 mb-1">
                    <form action="" method="GET">
                        ...
                        <h3>
                            Order Sales Report ({{ $start->format('d M Y') }} - {{ $end->format('d M Y') }})
                        ...
                        <table class="table table-bordered table-center">
                            ...
                            <tbody>
                                @foreach ($sales as $key => $row)
                                    <tr>
                                        ...
                                        <td>{{ $row['customer'] }}</td>
                                        <td>{{ $row['qty'] }}</td>
                                        <td>{{ $setting->currency_sign }} {{ number_format($row['sales'], 2) }}</td>
                                        <td>{{ $setting->currency_sign }} {{ number_format($row['purchase'], 2) }}</td>
                                        <td>{{ $setting->currency_sign }} {{ number_format($row['discount'], 2) }}</td>
                                        <td>{{ $setting->currency_sign }} {{ number_format($row['product_discount'], 2) }}
                                        </td>
                                        <td>{{ $setting->currency_sign }} {{ number_format($row['shipping'], 2) }}</td>
                                        <td class="{{ $row['profit'] >= 0 ? 'text-success' : 'text-danger' }}">
                                            {{ $setting->currency_sign }} {{ number_format($row['profit'], 2) }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                            ...
                            <tfoot>
                                <tr class="fw-bold">
                                    ...
                                    <td>{{ $setting->currency_sign }} {{ number_format($footer['total_discount'], 2) }}
                                    </td>
                                    <td>{{ $setting->currency_sign }}
                                        {{ number_format($footer['total_product_discount'], 2) }}
                                    </td>
                                    <td>{{ $setting->currency_sign }} {{ number_format($footer['total_shipping'], 2) }}
                                    </td>
