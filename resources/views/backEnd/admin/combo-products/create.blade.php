    ...
    <div class="dashboard-wrapper">
        <div class="dashboard-ecommerce">
            <div class="container-fluid dashboard-content ">
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('admin.combo-products.store') }}" method="post"
                            ...
                            <div class="mb-3">
                                ...
                                <select id="combo_products" class="form-control select2" name="combo_products[]" multiple
                                    ...
                                    @foreach ($products as $p)
                                        <option value="{{ $p->id }}">{{ $p->name }} (SKU: {{ $p->sku }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="card mb-3" id="productDetailsCard" style="display:none;">
...
@push('js')
        ...
        $(document).ready(function() {
            ...
            $('#gallery_image').change(function() {
                ...
                Array.from(this.files).forEach(file => {
                    let reader = new FileReader();
                    reader.onload = function(e) {
                        $('#galleryPreview').append('<img class="mb-2" src="' + e.target
                            .result +
                            '" width="50">');
                    }
    ...
    <script>
        $(function() {
                ...
                skuTimer = setTimeout(() => {
                    $.get("{{ route('admin.combo-products.check-sku') }}", {
                        ...
                        .done(function(res) {
                            console.log(res);
                            if (res.exists) {
                                $('#sku_status').text('SKU already exists — choose another')
            ...
            // fetch product details and populate table when selection changes
...
<td class="text-center"><button type="button" class="btn btn-sm btn-danger removeRow">X</button></td>
                        ...
                        // Show card now that products selected
                ...
                if (ids.length === 0) {
                    $('#comboDetailsTable tbody').html('');
                    recalcTotals();
                    $('#productDetailsCard').slideUp(200); // hide the section if no products selected
                    return;
                }
            ...
            // recalc when any input changes in the table
            $(document).on('input', '.cp_purchase, .cp_sale, .cp_regular, .cp_qty', function() {
                recalcTotals();
            });
            ...
            function recalcTotals() {
                ...
                $('#comboDetailsTable tbody tr').each(function() {
                    ...
                    tPurchase += (purchase * qty);
                    tRegular += (regular * qty);
                    tSale += (sale * qty);
                    tQty += qty;
                });
            ...
            // On form submit ensure SKU check passed
            $('#comboForm').on('submit', function(e) {
                const skuMsg = $('#sku_status').text().toLowerCase();
                if (skuMsg.includes('already exists')) {
                    e.preventDefault();
                    alert('SKU is already used. Choose another SKU.');
                }
            });
