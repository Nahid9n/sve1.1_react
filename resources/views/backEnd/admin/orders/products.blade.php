<tr class="product_{{ $data->id }}" data-product_id="{{ $data->id }}">
                    ...
                    @foreach ($variant->get_variant_items as $key2 => $variant_item)
                        <div class="form-check me-2">
                            <input class="form-check-input form-checked-outline attribute" type="radio"
                                name="variant[{{ $data->id }}][{{ strtolower($variant->get_variant->name) }}][]"
                                id="attribute_{{ $data->id }}_{{ strtolower($variant->get_variant->name) . '_' . $key2 }}"
                                value="{{ strtolower($variant_item->name) }}" {{ $key2 == 0 ? 'checked' : '' }}>
                            <label class="form-check-label"
                                for="attribute_{{ $data->id }}_{{ strtolower($variant->get_variant->name) . '_' . $key2 }}">
